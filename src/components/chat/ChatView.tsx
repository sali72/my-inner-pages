      ...
      <div className="w-full max-w-3xl mx-auto flex-1 flex flex-col min-h-0">
        ...
        <div className={`flex-1 rounded-2xl ${bgClass} border ${borderClass} overflow-hidden flex flex-col min-h-0`}>
          {messages.length === 0 ? (
            ...
            <div className={`flex-1 overflow-y-auto p-4 space-y-4 scrollbar-theme ${isDark ? 'scrollbar-theme-dark' : 'scrollbar-theme-light'}`}>
              {messages.map((msg) => (
                <div
                  key={msg.id}
          ...
          <div className={`border-t ${borderClass} p-4 shrink-0`}>
            ...
            <div className={`flex gap-3 items-end ${isDark ? 'bg-slate-800' : 'bg-white'} rounded-xl border ${borderClass} p-2`}>
              <textarea
                ...
                className={`flex-1 resize-none bg-transparent px-2 py-1.5 text-sm outline-none disabled:opacity-50 scrollbar-theme ${isDark ? 'scrollbar-theme-dark text-slate-200 placeholder:text-slate-400' : 'scrollbar-theme-light text-slate-800 placeholder:text-slate-500'}`}
